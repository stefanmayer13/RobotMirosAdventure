...
exports.checkIfMoveAllowed = function (pos, oldPos) {
  var level = stage.getStage(fileContent.stage, fileContent).level;
...
exports.isFinished = function (pos) {
  var finish = stage.getStage(fileContent.stage, fileContent).finish;
...
function getAdjacentTile (direction) {
  ...
  var type = pos[0] < 0 || pos[1] < 0 || level.length <= pos[1] || level[pos[1]].length <= pos[0] || level[pos[1]][pos[0]] ? 'wall' : 'empty';
  if (type === 'empty' && fileContent.markers && fileContent.markers[pos[0]] && fileContent.markers[pos[0]][pos[1]]) {
    type = 'marked';
  }
  return type;
}
